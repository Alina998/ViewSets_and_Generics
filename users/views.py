...
class SubscriptionCreateView(generics.CreateAPIView):
    ...
    ]  # Только авторизованные пользователи могут подписываться

    def perform_create(self, serializer):
        serializer.save(
            user=self.request.user
        )  # Устанавливаем текущего пользователя как создателя подписки
...
class SubscriptionListView(generics.ListAPIView):
    ...
    permission_classes = [
        permissions.IsAuthenticated
    ]  # Только авторизованные пользователи могут просматривать свои подписки
    pagination_class = CustomPageNumberPagination
...
class SubscriptionDeleteView(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        return Subscription.objects.get(
            user=self.request.user, course=self.kwargs["course_id"]
        )
